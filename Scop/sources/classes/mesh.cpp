...
{
	std::vector<vec3> new_vertexes/* = this->base_vertexes*/;
	std::vector<t_triangle> new_triangles;
	...
	for (std::vector<std::vector<unsigned int>>::iterator it = this->base_faces.begin(); it != this->base_faces.end(); ++it)
	{
		...
		{
			// t_triangle &triangle, int pos, std::vector<vec3> *new_vertexes,
...
t_triangle mesh::getSemioptiVerticesTriangle(t_triangle &triangle, std::size_t pos, std::vector<vec3> *new_vertexes,
						std::vector<t_triangle> *base_face_triangles, std::vector<t_triangle> *new_face_triangles)
{
	t_triangle ret;
	bool found = false;
...
void mesh::addInfoToVertexesEco()
{
	...
	for (std::size_t face = 0; face < this->vertByFace.size(); face++)
	{
		for (std::size_t vertInFace = 0; vertInFace < this->vertByFace[face].size(); vertInFace++)
		{
			...
			vec3 tmp = { clor, clor,clor};
			// vec3 tmp = colors[face % 3];
...
void mesh::min_max_bounds(vec3& min_bound, vec3& max_bound)
{
	for (std::vector<vec3>::iterator vert = this->vertexes.begin(); vert != this->vertexes.end();)  {
...
void	mesh::center_around_orgin()
{
	...
	for (std::vector<vec3>::iterator vert = this->vertexes.begin(); vert != this->vertexes.end(); ++vert)  {
		(*vert)[0] -= center[0];
		(*vert)[1] -= center[1];
		(*vert)[2] -= center[2];
	}
